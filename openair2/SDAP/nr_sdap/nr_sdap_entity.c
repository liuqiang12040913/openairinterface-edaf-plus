...
#if EDAF
    LATSEQ_P("D gtp.in--sdap.sdu", "len%u::qfi%u.drb%ld.Pbuf%u", sdu_buffer_size, qfi, sdap_drb_id, (uint8_t*)sdu_buffer);
#endif
  } else { // nrUE
    offset = SDAP_HDR_LENGTH;
    /*
                              ...
                              int size)
{
  ...
  if (is_gnb) { // gNB
...
#if EDAF
    int sn;
    memcpy(&sn, buf+size-offset, sizeof(sn));
    LATSEQ_P("U sdap.sdu--gtp.out", "len%u::SBuf%u.sn%u", size-offset, (uint8_t*)buf, sn);
#endif
  } else { //nrUE
    /*
     * TS 37.324 5.2 Data transfer
...
#if EDAF
    {
      int sn;
      memcpy(&sn, buf+size-offset, sizeof(sn));
      LATSEQ_P("D sdap.sdu--ip.out", "len%u::SBuf%u.sn%u", size-offset, (uint8_t*)buf, sn);
    }
#endif
