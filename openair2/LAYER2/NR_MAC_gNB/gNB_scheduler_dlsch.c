                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  UE_iterator(UE_info->list, UE) {
...
#if EDAF
            LATSEQ_P("D rlc.pdu--mac.sdu", "len%u::fm%u.sl%u.lcid%u.hqpid%u.M1buf%u.ENTno%d",
                     len, frame, slot, lcid, current_harq_pid,
                     (uint8_t*)buf+sizeof(NR_MAC_SUBHEADER_LONG), getEDAFDLTxpdu());
            incrementEDAFDLTxpdu();
#endif
...
#if EDAF
    LATSEQ_P("D mac.sdu--mac.harq", "len%u::fm%u.sl%u.hqpid%u.rnti%u.Hbuf%u",
             TBS, frame, slot, current_harq_pid, rnti, (uint8_t*)harq->transportBlock);
#endif
    ...
    TX_req->Slot = slot;
#if EDAF
    LATSEQ_P("D mac.harq--phy.tx", "len%u::fm%u.sl%u.hqpid%u.pduindex%u",
             TBS, frame, slot, current_harq_pid, pduindex);
#endif
