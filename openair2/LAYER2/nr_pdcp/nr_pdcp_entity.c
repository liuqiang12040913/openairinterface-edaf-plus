...
#if EDAF
  int sn;
  memcpy(&sn, buffer+size, sizeof(sn));
  // in file openair2/LAYER2/nr_pdcp/nr_pdcp_sdu.c
  // function nr_pdcp_new_sdu, we actually allocate 4bytes more
  // so here we copy sn value to that area
  memcpy(sdu->buffer+sdu->size, &sn, sizeof(sn));
  LATSEQ_P(entity->is_gnb ? "U pdcp.decoded--sdap.sdu" : "D pdcp.decoded--sdap.sdu", "len%u::PBuf%u.SBuf%u.sn%u", sdu->size, (uint8_t*)buffer ,(uint8_t*)sdu->buffer, sn);
#endif
...
#if EDAF
  LATSEQ_P(entity->is_gnb ? "D pdcp.sdu--pdcp.cipher" : "U pdcp.sdu--pdcp.cipher", "len%u::rb%d.sduid%d.Pbuf%u.PCbuf%u", header_size + size + integrity_size, entity->rb_id, sdu_id, (uint8_t*)buffer, (uint8_t*)buf);
#endif
