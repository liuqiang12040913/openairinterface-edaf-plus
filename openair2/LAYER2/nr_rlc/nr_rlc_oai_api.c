...
#if EDAF
  int sn;
  memblock = malloc16(size+sizeof(sn));
#else
...
#if EDAF
  memcpy(&sn, buf+NR_SDU_MAX, sizeof(sn));
  LATSEQ_P(is_enb ? "U pdcp.ind--pdcp.decoded" : "D pdcp.ind--pdcp.decoded", "len%u::PIBuf%u.PBuf%u.sn%u", size, buf, memblock, sn);
#endif
  memcpy(memblock, buf, size);
#if EDAF
  memcpy(memblock+size, &sn, sizeof(sn));
#endif
...
void nr_rlc_add_srb(int ue_id, int srb_id, const NR_RLC_BearerConfig_t *rlc_BearerConfig)
{
  ...
  if (ue->srb[srb_id-1] != NULL) {
    ...
    nr_rlc_entity_t *nr_rlc_am = new_nr_rlc_entity_am(RLC_RX_MAXSIZE,
                                                      ...
                                                      sn_field_length);
#if EDAF
    nr_rlc_am->is_gnb = nr_rlc_manager_get_enb_flag(nr_rlc_ue_manager);
#endif
...
static void add_drb_um(int ue_id, int drb_id, const NR_RLC_BearerConfig_t *rlc_BearerConfig)
{
  ...
  if (ue->drb[drb_id-1] != NULL) {
    ...
    nr_rlc_entity_t *nr_rlc_um = new_nr_rlc_entity_um(RLC_RX_MAXSIZE,
                                                      ...
                                                      sn_field_length);
#if EDAF
    nr_rlc_um->is_gnb = nr_rlc_manager_get_enb_flag(nr_rlc_ue_manager);
#endif
...
#if EDAF
  nr_rlc_tm->is_gnb = nr_rlc_manager_get_enb_flag(nr_rlc_ue_manager);
#endif
