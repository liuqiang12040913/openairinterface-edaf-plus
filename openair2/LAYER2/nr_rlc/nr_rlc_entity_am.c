...
#if EDAF
  #include "common/utils/EDAF/latseq.h"
#endif
...
static void reassemble_and_deliver(nr_rlc_entity_am_t *entity, int sn)
{
  nr_rlc_pdu_t *pdu;
#if EDAF
  // we piggyback the sn number to the data, so we take 4 more bytes here
  char sdu[NR_SDU_MAX+sizeof(sn)];
#else
  char sdu[NR_SDU_MAX];
#endif
  int so = 0;
  ...
  /* deliver */
#if EDAF
  // we piggyback the sn number to the data
  memcpy(sdu + NR_SDU_MAX, &sn, sizeof(sn));
  LATSEQ_P(entity->common.is_gnb ? "U rlc.reassembled--pdcp.ind" : "D rlc.reassembled--pdcp.ind", "len%u::sn%u.PIBuf%u", so, sn, sdu);
#endif
...
#if EDAF
  LATSEQ_P(entity->common.is_gnb ? "U rlc.decoded--rlc.reassembled" : "D rlc.decoded--rlc.reassembled", "len%u::MRbuf%u.p%u.si%u.sn%u.so%u", data_size, buffer, p, si, sn, so);
#endif
...
#if EDAF
  LATSEQ_P(entity->common.is_gnb ? "D rlc.queue--rlc.txpdu" : "U rlc.queue--rlc.txpdu",
           "len%u::leno%d.tbs%d.sn%u.pdulen%d.R2buf%u.M1buf%u.ENTno%d.retx", ret_size, sdu->size, size,
           sdu->sdu->time_of_arrival, pdu_size, (uint8_t*)sdu->sdu->data, (uint8_t*)buffer,
           entity->common.is_gnb ? getEDAFDLTxpdu() : getEDAFTxpdu());
#endif
...
#if EDAF
  LATSEQ_P(entity->common.is_gnb ? "D rlc.queue--rlc.txpdu" : "U rlc.queue--rlc.txpdu",
           "len%u::leno%d.tbs%d.sn%u.srn%d.pdulen%d.R2buf%u.M1buf%u.ENTno%d", ret_size, sdu->size, size,
           sdu->sdu->time_of_arrival, sdu->sdu->sn, pdu_size, (uint8_t*)sdu->sdu->data, (uint8_t*)buffer,
           entity->common.is_gnb ? getEDAFDLTxpdu() : getEDAFTxpdu());
#endif
                               ...
                               int sdu_id)
{
  ...
  LATSEQ_P(entity->common.is_gnb ? "D pdcp.pdu--rlc.queue" : "U pdcp.pdu--rlc.queue",
           "len%u::sduid%d.queue%d.sn%u.R1buf%u.R2buf%u", size, sdu_id, entity->tx_size-size,
           sdu->sdu->time_of_arrival, (uint8_t*)buffer, (uint8_t*)sdu->sdu->data);
#else
  if (entity->common.avg_time_is_on)
    sdu->sdu->time_of_arrival = time_average_now();
#endif
