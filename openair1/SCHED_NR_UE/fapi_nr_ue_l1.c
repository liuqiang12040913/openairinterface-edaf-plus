      ...
      case FAPI_NR_UL_CONFIG_TYPE_PUSCH: {
        ...
        if (pdu->pusch_config_pdu.tx_request_body.pdu) {
          LOG_D(PHY,
                ...
                current_harq_pid);
#if EDAF
	  // NOTE: our data that later we will look to track it, is inside this tx_request with size tx_request_body.pdu_length. Other payloads can be here too. So M2buf is not necessarily equal to the M2buf we record previously. It is important that [M2buf: M2buf+M2len] be inside [M3buf : M3buf+M3len].
	  // M3len = pdu->pusch_config_pdu.tx_request_body.pdu_length
	  LATSEQ_P("U mac.sdu--mac.harq","len%u::hqpid%d.fm%d.sl%d.M3buf%u.Hbuf%u", pdu->pusch_config_pdu.tx_request_body.pdu_length, current_harq_pid, ul_config->frame, ul_config->slot, (uint8_t*)pdu->pusch_config_pdu.tx_request_body.pdu, (uint8_t*)harq_process_ul_ue->payload_AB);
#endif
