...
#if EDAF
  LATSEQ_P("U mac.harq--phy.tx","len%u::rnti%x.nb_rb%d.nb_sym%d.mod_or%d.hqpid%d.fm%d.sl%d.Hbuf%u", pusch_pdu->pusch_data.tb_size, rnti, nb_rb, number_of_symbols, mod_order, harq_pid, frame, slot, (uint8_t*)harq_process_ul_ue->payload_AB);
#endif
